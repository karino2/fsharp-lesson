...
この重複しないという事のおかげで集合論的にリレーションを定義出来る、
...
この代数も集合論の言葉で記述出来るため、最小の基底の数だとか類似のモデル同士の表現力が等価である事が証明出来たりとか、
また複雑な合成を変形しても等価になるような最適化に使える変形などを調べる事が出来るというご利益がありました。

実装する側としては、一般的な（数学の意味での）ドメインに対して、ある種のフィルタを適用したものとしてリレーションが生成出来るという事に着目しておくと、
...
例で出てくるようなのは対応した上で、わかるものだけ受け入れる（怪しいものをはじくでは無くで安全なものを受け入れる）という風にパターンを書くと良いでしょう。
