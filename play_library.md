...
少し演習問題として、もう少し複雑なパーサー定義が何を返すのかを見てみましょう。
先の課題ですが、susumu2357氏が以下のようなコードを書いてきました。
...
最初のカッコでは"project"が変えるけれど、その次の`>>.`の合成で`"("`が返るのでこの"project"は捨てられます。
...
一度こうした構成を作る体験をしておくと同じようなプログラムを作る時にも応用が効くので、次の第二回でこうした構成の基本を体験する事にします。
